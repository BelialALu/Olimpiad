...
const questionsText = {
    ...
    "6": "Вопрос 6: Решите в целых числах уравнение $$6x^2-3xy+7y-23x+26=0$$. В ответ запишите сумму произведений всех найденных значений $$(x;y)$$. (Например, найденные решения – это две пары (1; 1) и (2; 3), тогда в ответ следует записать число 7, потому что 1 * 1 + 2 * 3 = 7.)",
    ...
    "8": "Вопрос 8: Точки L, E, F, T – последовательные вершины параллелограмма. На отрезке LT отмечена точка N такая, что LN : NT = 3 : 2. На отрезке LF отмечена точка O такая, что LO : OF = 2 : 3. Прямая NO пересекает отрезок EF в точке G. Найдите площадь четырехугольника LEGO, если площадь параллелограмма LEFT равна 100."
};
